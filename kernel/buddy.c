...
int bd_initfree_pair(int k, int bi, void *allow_start, void *allow_end)
{
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (bit_get(bd_sizes[k].alloc, bi))
  {
    ...
    free = BLK_SIZE(k);
    char *blk_addr = addr(k, buddy);
    if (blk_addr >= (char *)allow_start && blk_addr < (char *)allow_end)
...
int bd_initfree(void *bd_left, void *bd_right, void *allow_start, void *allow_end)
{
  int free = 0;
  ...
  { // skip max size
    ...
    free += bd_initfree_pair(k, left, allow_start, allow_end);
    if (right <= left)
      continue;
    free += bd_initfree_pair(k, right, allow_start, allow_end);
  }
  return free;
}
...
void bd_init(void *base, void *end)
{
  ...
  {
    sz = sizeof(char) * (ROUNDUP(NBLK(k), 8)) / 8; //分配split数组
    bd_sizes[k].split = p;
